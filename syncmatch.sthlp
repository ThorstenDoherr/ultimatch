...
variable {cmd:occ_raise} contains the first year when an occupational rank was improved. Although the variable {cmd:occ_code} is rather categorial, there is a slight
tendency to more independent, less routine work for lower numbers. The variable {cmd:occ_raise} is missing for women that never reported a change in their
occupation (the division with the dummy {cmd:switch} is missing for zero). This is the control group. We want to observe the change in 
the wage for the treated group. We apply Mahalanobis distance-based neighborhood matching on age, wage and job experience constrained by exact matching of 
industry, occupation and college education. 
...
periods. The system variable {cmd:_match} - generated by {help syncmatch:syncmatch} - constitutes the linkage between treated and counterfactual observations.{p_end}
...
{hline}
{phang2}{cmd:webuse nlswork, clear}{p_end}
{phang2}{cmd:gen p_treated = normal(-collgrad*0.25 + (occ_code == 8)*0.5 - ln_wage*0.25 - (age-16)/29 + invnorm(uniform())*0.5)}{p_end}
{phang2}{cmd:egen byte treated = max(p_treated > 0.6 & year == 72), by(idcode)}{p_end}
{phang2}{cmd:replace ln_wage = ln_wage+0.2 if treated & year > 72}{p_end}
{phang2}{cmd:syncmatch age ln_wage ttl_exp, time(year) start(73) treated(treated) exact(ind_code occ_code collgrad) event(ln_wage) unit(idcode) unm}{p_end}
{phang2}{cmd:syncevent ln_wage, time(year) start(73) treated(treated) unit(idcode) match(_match) window(-5 .) noc bal}{p_end}
{hline}
