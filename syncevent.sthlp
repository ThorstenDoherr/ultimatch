...
{pstd}We use the "National Longitudinal Survey of Young Women, 14-24 years old in 1968" survey (nlswork) data to emulate a staggered treatment event. The treatment 
variable {cmd:spouse} contains the first year when a spouse is reported present in the household signified by the dummy {cmd:msp}. The variable {cmd:spouse} is 
missing for single women that never reported a spouse (the division with the dummy is missing for zero). This is the control group. We want to observe the change in 
weekly work hours for the treated group. The matching is performed with the {help syncmatch:syncmatch} command applying Mahalanobis distance-based neighborhood 
...
periods. The system variable {cmd:_match} - generated by {help syncmatch:syncmatch} - constitutes the linkage between treated and counterfactual observations. We 
see that there is no difference between the unbalanced and the balanced model as the {cmd:_weight} variable indicates no multiple assignments of counterfactuals. 
The system variable {cmd:_hood} marks the panel observations matched by {help syncmatch:syncmatch}.{p_end}
...
{phang2}{cmd:syncevent hours, time(year) start(spouse) unit(idcode) match(_match) window(-5 .) nocohort}{p_end}
...
study instead. Furthermore, the elaborate matching for this example seems unnecessary considering the random nature of the treatment event. The last example will
provide a simple test for random selection.{p_end}
...
{hline}
{phang2}{cmd:webuse nlswork, clear}{p_end}
{phang2}{cmd:gen p_treated = normal(-collgrad*0.25 + (occ_code == 8)*0.5 - ln_wage*0.25 - (age-16)/29 + rnormal(0,0.5))}{p_end}
...
{phang2}{cmd:syncmatch hours age ln_wage ttl_exp, time(year) start(73) treated(treated) exact(ind_code occ_code collgrad) event(ln_wage) unit(idcode) unm}{p_end}
{phang2}{cmd:syncevent ln_wage, time(year) start(73) treated(treated) unit(idcode) window(-5 .) noc bal}{p_end}
{hline}

{pstd}Finally, we can test for random selection by randomly assigning counterfactuals using
score-based matching on random numbers. This test confirms that the pre-treatment trends are parallel (insignificant) without elaborate matching because of the random treatment assignment.{p_end}
